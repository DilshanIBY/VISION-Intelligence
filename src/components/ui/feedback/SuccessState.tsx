  ...
  (
    ...
    const renderIcon = () => {
      switch (variant) {
        case 'celebration':
          return (
            <PartyPopperIcon size={40} className="text-[var(--color-accent)]" strokeWidth={1.5} />
          );
        case 'minimal':
          return <CheckIcon size={32} className="text-[var(--color-success)]" strokeWidth={2} />;
        default:
          return (
            <CheckCircle2Icon size={40} className="text-[var(--color-success)]" strokeWidth={1.5} />
          );
      }
    };

    const iconBgColor =
      variant === 'celebration' ? 'rgba(245, 158, 11, 0.1)' : 'rgba(16, 185, 129, 0.1)';
  ...
  ({ size = 24, animate = true, className = '', ...props }, ref) => {
    return (
      <div
        ...
        {...props}
      >
        <CheckIcon size={size * 0.6} className="text-white" strokeWidth={3} />
      </div>
    );
  }
);
...
export const SuccessToastContent = ({ title = 'Success', description }: SuccessToastProps) => (
