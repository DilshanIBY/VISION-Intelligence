...
export const Breadcrumb = forwardRef<HTMLElement, BreadcrumbProps>(
    ({ items, separator, showHome = true, homePath = '/', className = '' }, ref) => {
        ...
        const allItems = showHome ? [{ label: 'Home', href: homePath, icon: <Home size={14} /> }, ...items] : items;
                    ...
                    return (
                        <div key={index} className="flex items-center gap-2">
                            ...
                            {item.href && !isLast ? (
                                ...
                                <span
                                    className={`flex items-center gap-1.5 text-sm ${isLast ? 'font-medium text-[var(--color-text-primary)]' : 'text-[var(--color-text-muted)]'
                                        }`}
                                >
                                    {item.icon}
                                    <span>{item.label}</span>
