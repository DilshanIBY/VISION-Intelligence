...
export const ScenarioCard = forwardRef<HTMLDivElement, ScenarioCardProps>(
    ({ scenario, baseline, onRemove, onRename, onSelect, isSelected = false, className = '' }, ref) => {
        const { outputs, isBaseline, name } = scenario;

        const machineDelta = baseline ? calculateDelta(outputs.machinesRequired, baseline.machinesRequired) : null;
        const costDelta = baseline ? calculateDelta(outputs.costEstimate, baseline.costEstimate) : null;
        const daysDelta = baseline ? calculateDelta(outputs.totalProductionDays, baseline.totalProductionDays) : null;
        ...
        const DeltaIndicator = ({ delta, inverseColor = false }: { delta: DeltaDisplay; inverseColor?: boolean }) => {
            ...
            const isGood = inverseColor
                ? delta.direction === 'up'
                : delta.direction === 'down';

            return (
                <span className={`flex items-center gap-0.5 text-xs font-medium ${isGood ? 'text-[var(--color-success)]' : 'text-[var(--color-error)]'}`}>
            ...
            >
                ...
                {isEditing ? (
                    <div className="mb-3 flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                        ...
                        />
                        <button onClick={handleSaveName} className="p-1 hover:bg-white/20 rounded-full text-[var(--color-success)]">
                            <Check size={14} />
                        </button>
                    </div>
                ) : (
                    <div className="group flex items-center justify-start gap-2 mb-3 pr-6">
                        <h4 className={`text-sm font-semibold ${isBaseline ? 'text-white' : 'text-[var(--color-text-primary)]'}`}>
                            {name}
                        </h4>
                        {!isBaseline && onRename && (
                            <button
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between">
                        <span className={`text-xs ${isBaseline ? 'text-white/70' : 'text-[var(--color-text-muted)]'}`}>
                            Machines
                        </span>
                        <div className="flex items-center gap-2">
                            <span className={`text-lg font-bold ${isBaseline ? 'text-white' : 'text-[var(--color-text-primary)]'}`}>
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                            <span className={`text-sm font-semibold ${isBaseline ? 'text-white' : 'text-[var(--color-text-primary)]'}`}>
