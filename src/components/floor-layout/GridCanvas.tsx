  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners} className="group">
      ...
      {isSelected && !isDragging && (
        <>
          <div
            className="absolute -right-1.5 -bottom-1.5 w-3 h-3 bg-white border-2 border-primary rounded-full cursor-nwse-resize z-50 shadow-sm"
  ...
  const findTargetAtPosition = useCallback(
    (x: number, y: number) => {
      ...
      for (let i = canvasObjects.length - 1; i >= 0; i--) {
        ...
        if (
          obj.floorIndex === activeFloorIndex &&
          x >= obj.x &&
          x <= obj.x + obj.width &&
          y >= obj.y &&
          y <= obj.y + obj.height
          ...
          if (obj.type !== 'arrow' && obj.type !== 'curved-arrow')
            return {
              type: 'object',
              id: obj.id,
      ...
      for (const dept of floorDepartments) {
        ...
        if (x >= left && x <= right && y >= top && y <= bottom) {
          return {
            type: 'department',
            id: dept.id,
            cx: left + (dept.width * GRID_CELL_SIZE) / 2,
            cy: top + (dept.height * GRID_CELL_SIZE) / 2,
          };
        }
      }
      return null;
          ...
          type:
            activeTool === 'shape-rect'
              ...
              : activeTool === 'shape-circle'
                ...
                : activeTool === 'note'
                  ? 'note'
                  : activeTool === 'arrow' || activeTool === 'curved-arrow'
                    ? activeTool
                    : 'text',
        });
      }
  ...
  const handleMouseUp = useCallback(() => {
    ...
    if (creationStart && tempObject) {
      ...
      if (activeTool === 'arrow' || activeTool === 'curved-arrow') {
        ...
        const p2 = {
          x: isDraggedRight ? tempObject.x! + w : tempObject.x!,
      ...
      if (
        ...
        onAddObject({
          ...
          y: tempObject.y!,
          width:
            isClick && (activeTool === 'arrow' || activeTool === 'curved-arrow')
              ? 100
              : activeTool === 'note' && isClick
                ? 150
                : isClick
                  ? 200
                  : tempObject.width!,
          ...
          startPoint:
            activeTool === 'arrow' || activeTool === 'curved-arrow' ? startPoint : undefined,
    ...
    >
      ...
      >
        ...
        <div className="absolute bottom-2 left-2 flex items-center gap-2 px-2 py-1 bg-white/80 dark:bg-slate-800/80 rounded backdrop-blur-sm">
          ...
          <span className="text-[10px] font-medium text-text-secondary">{GRID_CELL_METERS}m</span>
        </div>
        ...
        {/* Canvas Objects */}
        {canvasObjects
          .filter(o => o.floorIndex === activeFloorIndex)
          .map(obj => (
            <CanvasObjectItem
              key={obj.id}
              object={obj}
      ...
      {floorDepartments.length === 0 && canvasObjects.length === 0 && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="text-center p-6 bg-white/80 dark:bg-slate-800/80 rounded-2xl backdrop-blur-sm">
            ...
            <p className="text-xs text-text-muted mt-1">Drag departments or use tools</p>
          </div>
        </div>
      )}
    </div>
