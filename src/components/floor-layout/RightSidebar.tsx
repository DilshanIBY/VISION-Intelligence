    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          {/* Sidebar */}
          <motion.aside
            className={`fixed right-0 top-0 bottom-0 w-80 !bg-white dark:!bg-slate-900 shadow-2xl z-50 flex flex-col border-l border-glass-border ${className}`}
            initial={{ x: '100%' }}
            animate={{ x: 0 }}
            exit={{ x: '100%' }}
          ...
          >
            {/* Header */}
            <div className="flex items-center justify-between px-5 py-4 border-b border-glass-border bg-white/50 dark:bg-white/5">
